...
public class ShooterCalc implements Subsystem {
    ...
    public static Double[] calculateShotVectorandUpdateHeadingWithAcceleration(double robotHeading, Vector robotToGoalVector, Vector robotVel, Vector robotAccel){
        double g = 32.174*12; // Gravity
        double x = robotToGoalVector.getMagnitude()-ShooterConstants.PASS_THROUGH_POINT_RADIUS;
        double temp = x/39.37; // Convert to meters
        //double y = -4.5745*temp*temp*temp + 25.978*temp*temp - 48.395*temp + 58.675;
        ...
        double flywheelSpeed = Math.sqrt(g * x * x / (2 * Math.pow(Math.cos(hoodAngle), 2) * (x * Math. tan(hoodAngle) - y)));
        Vector robotVelocity = robotVel.plus(robotAccel.times(predictTime));
